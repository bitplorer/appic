...
wedge grain, biscuit fire, raku quench, the last coal, and the coda.
"""
...
WARP = ("wedge", "brief", "wheel", "bisque", "glaze", "make", "kiln", "raku", "watch", "air", "vitrine", "kintsugi", "gift", "coda")
WARP_HREF = {
    "wedge": "/wedge",
    "brief": "/brief",
    "wheel": "/wheel",
...
BISCUITS = ("bone", "cream", "toast")
RAKU_ATMS = ("oxidation", "reduction", "post")
COALS = ("live", "dark", "ash")
CODAS = ("rest", "return", "silence")
ROOM_PITCH = {
    "table": "G",
    "now": "A",
    "vessel": "E",
...
def ember_d(coal: str) -> str:
    """Last coal as a path. Live is denser. Ash is still. EMBER-3."""
    key = str(coal or "live")
    if key == "live":
        return "M0 22 Q12 4 24 22 T48 22 T72 22 T96 22 T120 22 T144 22 T168 22 T192 22 T216 22"
    if key == "dark":
        return "M0 22 Q28 12 56 22 T112 22 T168 22 T224 22"
...
class Host:
    ...
    grain: str = "medium"
    fold_n: int = 0
    wedges: list[dict[str, Any]] = field(default_factory=list)
    biscuit: str = "bone"
    bisque_remain: int = 0
    raku_atm: str = "oxidation"
    ash: int = 0
    coal: str = "live"
    coda_close: str = "rest"
    ...
    def name_grain(self, grain: str = "medium") -> str:
        """Name the clay grain. WEDGE-1."""
        grain = grain if grain in GRAINS else "medium"
        self.grain = grain
        self.notice = f"The grain is named {grain}."
        self.log("wedge.name", grain)
        self.score_write("wedge.name", "wedge", "F")
        return grain

    def knead(self) -> int:
        """One fold. Quantity stays Host stock. WEDGE-1."""
        self.fold_n = int(self.fold_n or 0) + 1
        row = {
            "id": f"w{self.fold_n:03d}",
            "grain": str(self.grain or "medium"),
            "at": _now(),
        }
        self.wedges.append(row)
        self.wedges = self.wedges[-24:]
        if self.vessel:
            self.seat(
                stage="brief",
                clay=str(self.vessel.get("clay") or "porcelain"),
                glaze=str(self.vessel.get("glaze") or "ash"),
                note=f"{self.grain} grain, {self.fold_n} folds",
                piece_id=str(self.vessel.get("id") or ""),
            )
        else:
            self.seat(stage="brief", clay="porcelain", glaze="ash", note=f"{self.grain} grain")
        self.notice = f"Fold {self.fold_n} on {self.grain} grain."
        self.log("wedge.knead", str(self.fold_n))
        self.score_write("wedge.knead", "wedge", "F")
        return int(self.fold_n)

    def name_biscuit(self, biscuit: str = "cream") -> str:
        """Name the first fire. BISQUE-1."""
        biscuit = biscuit if biscuit in BISCUITS else "cream"
        self.biscuit = biscuit
        self.notice = f"The biscuit is named {biscuit}."
        self.log("bisque.name", biscuit)
        self.score_write("bisque.name", "bisque", "D")
        return biscuit

    def light_bisque(self) -> int:
        """Light a cooler fire. Shared hearth, not glaze kiln. BISQUE-1."""
        self.bisque_remain = 6
        self.firing = True
        self.heat_remain = max(int(self.heat_remain or 0), 6)
        self.heat_trace.append(int(self.heat_remain))
        self.heat_trace = self.heat_trace[-24:]
        self.seat(stage="thrown", note=f"biscuit {self.biscuit}")
        self.notice = "The biscuit is lit."
        self.log("bisque.light", str(self.biscuit))
        self.score_write("bisque.light", "bisque", "D")
        return int(self.bisque_remain)

    def tick_bisque(self) -> int:
        if self.bisque_remain <= 0:
            self.biscuit = "toast"
            return 0
        self.bisque_remain = max(0, int(self.bisque_remain) - 1)
        self.tick_heat()
        if self.bisque_remain <= 0:
            self.biscuit = "toast"
            self.notice = "The biscuit is named toast."
            self.seat(stage="thrown", note="biscuit toast")
            self.log("bisque.drawn", "toast")
            self.score_write("bisque.drawn", "bisque", "D")
        return int(self.bisque_remain)

    def name_raku(self, atm: str = "reduction") -> str:
        """Name the quench atmosphere. RAKU-1."""
        atm = atm if atm in RAKU_ATMS else "reduction"
        self.raku_atm = atm
        self.notice = f"The quench is named {atm}."
        self.log("raku.name", atm)
        self.score_write("raku.name", "raku", "B")
        return atm

    def quench(self) -> str:
        """Sudden quench. Heat dumps to ash. RAKU-1."""
        self.raku_atm = "post"
        dumped = int(self.heat_remain or 0)
        self.ash = int(self.ash or 0) + max(1, dumped)
        self.heat_remain = 0
        self.firing = False
        self.coal = "ash"
        self.heat_trace.append(0)
        self.heat_trace = self.heat_trace[-24:]
        if self.vessel:
            self.draw(
                {
                    "clay": self.vessel.get("clay") or "clay",
                    "glaze": self.vessel.get("glaze") or "glaze",
                    "note": f"raku {self.raku_atm}",
                    "band": "done",
                    "id": self.vessel.get("id"),
                }
            )
        self.press_seal("raku.quench", "raku.quench")
        self.notice = f"Quenched. {self.ash} ash on the hearth."
        self.log("raku.quench", str(self.ash), "cap")
        self.score_write("raku.quench", "raku", "B")
        return "post"

    def name_coal(self, coal: str = "live") -> str:
        """Name the last coal. EMBER-3."""
        coal = coal if coal in COALS else "live"
        self.coal = coal
        if coal == "ash":
            self.heat_remain = 0
            self.firing = False
        elif coal == "live" and self.heat_remain <= 0:
            self.heat_remain = 2
            self.firing = True
        self.notice = f"The coal is named {coal}."
        self.log("ember.name", coal)
        self.score_write("ember.name", "ember", "C")
        return coal

    def name_coda(self, close: str = "rest") -> str:
        """Close the loop. CODA-1."""
        close = close if close in CODAS else "rest"
        self.coda_close = close
        if close == "return":
            self.occupy("table")
        if close == "silence":
            dropped = list(self.occupied[1:])
            self.occupied = self.occupied[:1] or ["coda"]
            for ghost in dropped:
                if ghost not in self.phantom:
                    self.phantom.insert(0, ghost)
            self.phantom = self.phantom[:12]
        self.notice = f"The coda is named {close}."
        self.log("coda.name", close)
        self.score_write("coda.name", "coda", "G")
        return close
...
def _seed(host: Host) -> Host:
    ...
    host.fugue_station = "wedge"
    host.phantom = ["kiln", "watch"]
    host.grain = "medium"
    host.fold_n = 3
    host.biscuit = "cream"
    host.raku_atm = "oxidation"
    host.coal = "live"
    host.coda_close = "rest"
