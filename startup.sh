...
if [ ! -d /workspace/.pydeps/fastapi ]; then
  if command -v uv >/dev/null 2>&1; then
    uv pip install --python "$PY" --target /workspace/.pydeps fastapi 'uvicorn[standard]' marko itsdangerous python-multipart || true
  else
    python3 -m pip install -q --target /workspace/.pydeps fastapi 'uvicorn[standard]' marko itsdangerous python-multipart || true
  fi
fi
